...
%     end
    ...
    for bounding_box_index = 1:size(bbox,1)
        ...
        raw_command_line = sprintf('%s %s %s "[%d,%d,%d,%d,%d,%d]" %s %s', ...
                          compiled_function_front_end_script_path, ...
        ...
        bsub_command_line = sprintf('bsub -P mouselight -n%d -R"affinity[core(1)]" -We %d -J %s -o %s ''%s''', ...
                               numcores, ...
